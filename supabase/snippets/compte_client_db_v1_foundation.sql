...
-- usage cible: nouvelle base `compte_client_db_dev` puis `compte_client_db_prod`
-- ce script est non destructif et idempotent autant que possible.
...
-- seed role operationnel by default helper (optional usage)
-- insert into core.roles_compte (compte_erp_id, code, libelle)
-- values ('<compte_erp_id>', 'operationnel', 'Operationnel')
