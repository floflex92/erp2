...
const NAV = [
  ...
  { to: '/planning', page: 'planning', label: 'Planning', icon: 'Planning' },
  { to: '/transports', page: 'transports', label: 'Ordres de transport', icon: 'OT' },
  { to: '/chauffeurs', page: 'chauffeurs', label: 'Conducteurs', icon: 'RH' },
  { to: '/vehicules', page: 'vehicules', label: 'Vehicules', icon: 'Parc' },
  { to: '/clients', page: 'clients', label: 'Clients', icon: 'CRM' },
  { to: '/facturation', page: 'facturation', label: 'Facturation', icon: 'Factu' },
  { to: '/tachygraphe', page: 'tachygraphe', label: 'Tachygraphe', icon: 'Tachy' },
  { to: '/parametres', page: 'parametres', label: 'Parametres', icon: 'Reglages' },
  { to: '/utilisateurs', page: 'utilisateurs', label: 'Utilisateurs', icon: 'Acces' },
]
    ...
    <aside className="w-64 min-h-screen bg-slate-900 text-white flex flex-col shrink-0">
      ...
      {isAdmin && (
        <div className="mx-3 mt-3 px-3 py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
          ...
          {isSimulating ? (
            ...
            <p className="text-yellow-200/60 text-[10px]">Acces complet</p>
          )}
          <button
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto mt-1">
        {NAV.filter(item => canAccess(displayRole, item.page)).map(item => (
          ...
          >
            <span className="inline-flex min-w-11 justify-center rounded-md bg-white/5 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-300">
              {item.icon}
            </span>
