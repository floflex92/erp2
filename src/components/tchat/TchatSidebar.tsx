    ...
    <div className={`flex w-full shrink-0 flex-col border-b md:w-[23rem] md:border-b-0 md:border-r ${className ?? ''}`} style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
      <div className="flex items-center justify-between border-b px-4 py-4" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        ...
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-xl border text-slate-400 transition-colors hover:border-white/20 hover:text-white"
      ...
      <div className="border-b px-3 py-3" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2.5">
        ...
        {normalizedSearch && (
          <p className="mt-2 text-[11px] text-slate-500">{filteredConvs.length} discussion(s) correspondante(s)</p>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 border-b px-3 py-2" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        <button
          ...
          className={`rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors ${filterRole === 'tous' ? 'bg-blue-600 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}
        >
          Tous
        </button>
        {roleOptions.map(option => {
          ...
          return (
            <button
              ...
              className={`rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors ${filterRole === option ? `${color.bg} ${color.text} ring-1 ring-white/20` : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}
            >
        ...
        <div className="border-b" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
          <div className="space-y-3 px-3 py-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Nouvelle discussion</p>
                <p className="mt-1 text-xs text-slate-400">Choisis des utilisateurs ou ajoute plusieurs groupes metier.</p>
              </div>
              <button type="button" onClick={resetComposer} className="text-[11px] font-medium text-slate-400 transition-colors hover:text-slate-200">Vider</button>
            </div>
              ...
              {composerRoleOptions.map(group => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-full px-2.5 py-1 text-[11px] font-medium transition-colors ${active ? `${color.bg} ${color.text} ring-1 ring-white/20` : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}
                  >
                    {ROLE_LABELS[group]}
                  </button>
                )
            ...
            <div className="flex items-center gap-2">
              ...
              {groupSelectionCount > 0 && <span className="text-[11px] text-slate-500">{groupSelectionCount} utilisateur(s)</span>}
            </div>
            ...
            <div className="nx-scrollbar max-h-56 space-y-1 overflow-y-auto">
              {composerProfiles.length === 0 && <p className="px-1 text-xs text-slate-500">Aucun utilisateur disponible</p>}
              {composerProfiles.map(candidate => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => toggleRecipient(candidate.id)}
                    className={`flex w-full items-center gap-2.5 rounded-xl px-2 py-2 text-left transition-colors ${selected ? 'bg-blue-600/12 ring-1 ring-blue-500/30' : 'hover:bg-white/6'}`}
                  >
                    ...
                    <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold ${selected ? 'border-blue-400 bg-blue-500 text-white' : 'border-white/10 text-slate-500'}`}>
                      {selected ? '✓' : '+'}
                    </span>
                  </button>
                )
      ...
      <div className="nx-scrollbar flex-1 overflow-y-auto">
        {loadingConvs ? (
          ...
          <div className="flex flex-col items-center justify-center px-4 py-16 text-center">
            ...
            <p className="text-sm text-slate-500">{normalizedSearch ? 'Aucun resultat' : 'Aucune conversation'}</p>
            <p className="mt-1 text-xs text-slate-600">{normalizedSearch ? 'Essaie un nom, un role, un texte ou un lien partage.' : 'Clique sur + pour demarrer'}</p>
          </div>
        ) : (
            ...
            {filteredConvs.map(conversation => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-start gap-3 rounded-xl px-3 py-3 text-left transition-all ${isActive ? 'bg-blue-600/20 ring-1 ring-blue-500/40' : 'hover:bg-white/5'}`}
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        {important && <span className="rounded-full bg-amber-400/20 px-2 py-0.5 text-[10px] font-semibold text-amber-100">Important</span>}
                        <span className="shrink-0 text-[10px] text-slate-500">{formatTime(conversation.updated_at)}</span>
                      </div>
                    </div>
                    <p className="mt-0.5 truncate text-[11px] text-slate-500">{conversationSubtitle(conversation)}</p>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      {presence && <TchatPresenceBadge presence={presence} compact />}
                      <p className="min-w-0 flex-1 truncate text-xs text-slate-500">{conversation.last_message ?? 'Nouvelle conversation'}</p>
